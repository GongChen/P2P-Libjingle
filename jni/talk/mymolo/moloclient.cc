...
void MoloClient::CreateP2P(const std::string& line) {
	worker_thread_->PostDelayed(1000, this, MSG_CREATECHANNEL,
			new MoloMessage(line));
}
...
void MoloClient::OnMessage(talk_base::Message *msg) {
	...
	case (MSG_CREATECHANNEL): {
		...
		//remote_user->value                
		this->to = "gc1@molohui.com";
		channel = new cricket::P2PTransportChannel("gc2@molohui.com",
...
void MoloClient::OnCandidate(cricket::TransportChannelImpl* ch,
	...
	if (to.length() > 0) {
		const std::string remoteUser = to;
